...
if ~isempty(goodnodes);
    ts.DD=load(goodnodes)+1;
else
    ts.DD=1:(ts.Nnodes)
end
...
netmat1=nets_makemats(ts,'corr');       % full correlation (normalised covariances)
save(output)
display('corr')

netmat2=nets_makemats(ts,'icov');       % partial correlation
save(output)
display('icov')

netmat3=nets_makemats(ts,'icov',0.1);   % regularised partial, with lambda=0.1
save(output)
display('icov.1')

netmat4=nets_makemats(ts,'icov',1);     %                                 =1
save(output)
display('icov1')

netmat5=nets_makemats(ts,'icov',10);    %                                 =10
save(output)
display('icov10')

netmat6=nets_makemats(ts,'icov',20);    %                                 =10
netmat6z=nets_r2z(ts,netmat6)  ;   %                                 =10
save(output)

netmat7=nets_makemats(ts,'icov',40);    %                                 =10
netmat7z=nets_r2z(ts,netmat7)    ; %                                 =10
save(output)

netmat8=nets_makemats(ts,'icov',60);    %                                 =10
netmat8z=nets_r2z(ts,netmat8)   ;  %                                 =10
save(output)

netmat9=nets_makemats(ts,'icov',80);    %                                 =10
netmat9z=nets_r2z(ts,netmat9)  ;   %                                 =10
save(output)

netmat10=nets_makemats(ts,'icov',100);    %                                 =10
netmat10z=nets_r2z(ts,netmat10);     %                                 =10
save(output)
...
netmat1z=nets_r2z(ts,netmat1)        % full correlation (normalised covariances)
netmat2z=nets_r2z(ts,netmat2)        % partial correlation
netmat3z=nets_r2z(ts,netmat3)    % regularised partial, with lambda=0.1
netmat4z=nets_r2z(ts,netmat4)      %                                 =1
netmat5z=nets_r2z(ts,netmat5)     %                                 =10
netmat11z=nets_r2z(ts,netmat11)                   % Hyvarinen's pairwise causality measure
